...
const bin = existsSync('./vendor/bin/docara') ? normalize('./vendor/bin/docara') : hasbin('docara', (result) => {
    if (result) return 'docara';
    console.error('Could not find Docara; please install it with Composer.');
    process.exit(1);
});

const env = process.env.NODE_ENV === 'development' ? 'local' : process.env.NODE_ENV;

const docara = () => spawn(bin, ['build', env], { stdio: 'inherit', shell: true }).on('exit', (code) => {
    if (code > 0) {
        console.warn(`\nDocara build failed, see above.`);
    }
});
...
mix.extend('docara', new class {
    ...
    webpackPlugins() {
        ...
        return [new class {
            apply(compiler) {
                compiler.hooks.afterCompile.tapAsync('DocaraWatchPlugin', watch(watchConfig));
                compiler.hooks.afterDone.tap('DocaraBuildPlugin', docara);
            }
        }];
    }
});
